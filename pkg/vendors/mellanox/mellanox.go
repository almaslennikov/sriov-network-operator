...
type MellanoxInterface interface {
	...
	MlxResetFW(pciAddresses []string, mellanoxNicsStatus map[string]map[string]sriovnetworkv1.InterfaceExt, hostHelper host.HostManagerInterface) error
}
...
func (m *mellanoxHelper) MlxResetFW(pciAddresses []string, mellanoxNicsStatus map[string]map[string]sriovnetworkv1.InterfaceExt, hostHelper host.HostManagerInterface) error {
	log.Log.Info("mellanox-plugin resetFW()")
	var errs []error
	for _, pciAddress := range pciAddresses {
		err := hostHelper.SetSriovNumVfs(pciAddress, 0)
		if err != nil {
		...
		if IsDualPort(pciAddress, mellanoxNicsStatus) {
			...
			err := hostHelper.SetSriovNumVfs(otherPortPCIAddress, 0)
			if err != nil {
