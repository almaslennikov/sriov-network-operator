...
var _ = Describe("SRIOV", func() {
	...
	Context("MlxResetFW", func() {
		...
		It("should reset VFs and firmware for single-port NIC", func() {
			...
			err := m.MlxResetFW([]string{"0000:d8:00.0"}, mellanoxNicsStatus, mockHostHelper)
			Expect(err).ToNot(HaveOccurred())
		})
		...
		It("should handle multiple single-port NICs", func() {
			...
			err := m.MlxResetFW([]string{"0000:d8:00.0", "0000:d9:00.0"}, mellanoxNicsStatus, mockHostHelper)
			Expect(err).ToNot(HaveOccurred())
		})
